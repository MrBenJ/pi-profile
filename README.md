...
[![verify](https://github.com/MrBenJ/pi-profile/actions/workflows/verify.yml/badge.svg)](https://github.com/MrBenJ/pi-profile/actions/workflows/verify.yml)
...
Manager JSON writes use an owned temporary file plus atomic rename, which protects against ordinary process interruption. Version 1.0.1 does not fsync the file and parent directory and therefore does **not** promise recovery across sudden power loss, kernel failure, or storage-controller failure.
...
See [docs/manual-smoke.md](docs/manual-smoke.md) for credential and cross-platform checks. Tests always use temporary synthetic fixtures. The authorized 1.0.1 release target is macOS; Windows/Linux checks are advisory rather than release gates.
