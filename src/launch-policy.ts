...
const managementCommands = new Set(["config", "install", "remove", "list", "update"]);
const managementFlags = new Set(["--help", "-h", "--version", "-v", "--list-models"]);
...
export function classifyInvocation(args: string[]): InvocationKind {
  ...
  const first = args[0];
  if (first && (managementCommands.has(first) || managementFlags.has(first))) return "management";
  return "session";
}
